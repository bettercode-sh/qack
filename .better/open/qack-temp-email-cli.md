...
- **Section 5.3 partial.** Root README and CI workflow added. npm name `qack` is **available** (404 on registry). `@qack/shared` moved to `devDependencies` in `qack` so `npm pack` ships only `dist/` + `package.json`. Publish with `pnpm --filter qack publish` after `npm login`.
